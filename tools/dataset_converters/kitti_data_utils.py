...
def get_vod_label_anno(label_path):
    """Parse VOD labels without using KITTI truncated semantics.

    VOD's second label column is a track id in newer annotations. This helper
    keeps it as ``track_id`` and deliberately omits ``truncated``.
    """
    annotations = {}
    annotations.update({
        'name': [],
